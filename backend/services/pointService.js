...
// The admin account is neutral: no points, no badges, no ranking. Every
// award path short-circuits for admins.
...
const awardStreakBonusInner = (memberId, callback) => {
    const sql = `
...
const awardGitHubPointsInner = (memberId, stats, callback) => {
...
const pointService = {
    awardAttendancePoints: (meetingId, memberId, status, callback) => {
        isAdminMember(memberId, (err, isAdmin) => {
            if (err || isAdmin) return callback(err || null);
    ...
    awardStreakBonus: (memberId, callback) => {
        isAdminMember(memberId, (err, isAdmin) => {
            if (err || isAdmin) return callback(err || null);
            awardStreakBonusInner(memberId, callback);
        });
    },
    ...
    awardGitHubPoints: (memberId, stats, callback) => {
        isAdminMember(memberId, (err, isAdmin) => {
            if (err || isAdmin) return callback(err || null);
            awardGitHubPointsInner(memberId, stats, callback);
        });
    },
